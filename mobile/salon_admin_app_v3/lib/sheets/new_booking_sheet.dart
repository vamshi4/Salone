...
class NewBookingSheet extends StatefulWidget {
  const NewBookingSheet({super.key, required this.salon, required this.staffRelations});
...
class _NewBookingSheetState extends State<NewBookingSheet> {
  ...
  List<Map<String, dynamic>> get _activeStylists =>
      widget.staffRelations.map((r) => r['stylist']).whereType<Map<String, dynamic>>().toList();
                  ...
                  children: [
                    if (_selectedServices.isNotEmpty)
                      Container(
                        ...
                        decoration: BoxDecoration(color: const Color(0xFF085041), borderRadius: BorderRadius.circular(AppRadius.md)),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text(
                                '${t.completedServicesCount(_selectedServices.length)}${_stylist != null ? ' · ${_stylist!['user']?['name'] ?? ''}' : ''}',
                                style: const TextStyle(color: Color(0xFF9FE1CB), fontSize: 12, fontWeight: FontWeight.w600)),
