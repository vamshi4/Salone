...
class AccountScreen extends StatefulWidget {
  const AccountScreen({super.key, required this.onLogout, required this.data});
...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      _dailyGoalController.text =
          (goal is int && goal > 0) ? (goal / 100).toStringAsFixed(0) : '';
  ...
  Future<void> _saveProfile() async {
    ...
    final goalMajor = int.tryParse(_dailyGoalController.text.trim());
    ...
    try {
      await api().patch('/api/v2/auth/me', data: {
        ...
        // Empty field = no goal set; 0 reads the same as "not set" on Home.
        'dailyRevenueGoal': (goalMajor == null || goalMajor <= 0) ? 0 : goalMajor * 100,
      });
      if (mounted) await widget.data.load();
  ...
  String _bookingLink(String salonId) => '$baseUrl/book/$salonId';

  Future<void> _copyLink(String salonId) async {
    await Clipboard.setData(ClipboardData(text: _bookingLink(salonId)));
    _show(t.linkCopied);
  }

  Future<void> _shareLink(String salonId, String salonName) async {
    await SharePlus.instance.share(
      ShareParams(text: t.bookingLinkShareText(salonName, _bookingLink(salonId))),
    );
  }
        ...
        child: _loading
            ...
            : ListView(
                ...
                children: [
                  ...
                  Text(t.bookingLinkSectionTitle, style: const TextStyle(color: AppColors.inkMuted, fontSize: 12, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 4),
                  Text(t.bookingLinkSectionSubtitle, style: const TextStyle(color: AppColors.inkMuted, fontSize: 12)),
                  const SizedBox(height: 8),
                  ...widget.data.salons.map((salon) => Padding(
                        padding: const EdgeInsets.only(bottom: 10),
                        child: _bookingLinkCard(salon),
                      )),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: t.dailyRevenueGoalLabel,
                      helperText: t.dailyRevenueGoalHint,
  ...
  Widget _bookingLinkCard(Map<String, dynamic> salon) {
    final salonId = '${salon['id']}';
    final salonName = '${salon['name'] ?? ''}';
    final link = _bookingLink(salonId);
    return Container(
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: QrImageView(data: link, version: QrVersions.auto, size: 72, gapless: false),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(salonName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                const SizedBox(height: 2),
                Text(link, style: const TextStyle(color: AppColors.inkMuted, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 8),
                Row(
                  children: [
                    TextButton.icon(
                      onPressed: () => _copyLink(salonId),
                      icon: const Icon(Icons.copy_outlined, size: 16),
                      label: Text(t.copyLinkButton),
                    ...
                    const SizedBox(width: 12),
                    TextButton.icon(
                      onPressed: () => _shareLink(salonId, salonName),
