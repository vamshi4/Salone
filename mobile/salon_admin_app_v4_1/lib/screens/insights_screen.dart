      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: salon == null
                  ? PickBranchPlaceholder(text: t.pickBranchFirst)
                  : (_tab == 0 ? _EarningsTab(salon: salon) : _RetentionTab(salon: salon)),
            ),
          ],
        ),
      ),
  ...
  Future<void> _export() async {
    ...
    try {
      ...
      final res = await api().get(
        '/api/v2/salons/$id/earnings/export?period=$_period',
        options: Options(responseType: ResponseType.bytes),
      );
      await SharePlus.instance.share(ShareParams(files: [
        XFile.fromData(Uint8List.fromList(res.data as List<int>),
            name: 'earnings-$_period.csv', mimeType: 'text/csv'),
      ]));
    } catch (e) {
      if (kDebugMode) debugPrint('Earnings export failed: $e');
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(AppLocalizations.of(context)!.couldNotExportEarnings)));
      }
    } finally {
          ...
          if (_loading)
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(_periodLabel(t), style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.w600, fontSize: 14)),
                  const SizedBox(height: 6),
                  Text(formatMoney(total),
                      style: Theme.of(context).textTheme.displaySmall?.copyWith(color: Colors.white, fontSize: 34)),
...
class _EarningsBars extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // With ~30 bars (month view) a label under every single bar has no room
    // to fit even a 2-digit day number at a legible size and wraps, which is
    // what caused the overflow — thin it out so only every Nth bar (and
    // always the last one) gets a label underneath.
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: daily.asMap().entries.map((entry) {
          ...
          final showLabel = i % labelStep == 0 || i == daily.length - 1;
          final label = date == null || !showLabel ? '' : '${date.day}';
          return Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Container(
                    height: 72 * ratio.clamp(0.03, 1.0),
                    decoration: BoxDecoration(
                      color: AppColors.accent,
...
class _ComparisonChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isUp = change >= 0;
    final color = isUp ? const Color(0xFF8FD9A8) : const Color(0xFFF0A99E);
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(t.customerServicePair(customer, service),
...
class _RetentionTabState extends State<_RetentionTab> {
  ...
  void _openProfile(Map<String, dynamic> c) {
    ...
    final allBookings = DashboardScope.of(context).bookings;
        ...
        children: [
          ...
          if (selected != null) ...[
            ...
            if (selected.members.isEmpty)
              ...
              ...selected.members.map((m) => _CohortMemberTile(
                  customer: Map<String, dynamic>.from(m as Map),
                  onRemind: _remind,
                  showRemind: selected.key == 'churned',
                  onTap: _openProfile)),
          ],
          const SizedBox(height: 20),
          Row(
          ...
          if (missed.isEmpty)
            ...
            ...missed.take(10).map((m) => _CohortMemberTile(customer: m, onRemind: _remind, showRemind: true, onTap: _openProfile)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          FilledButton.icon(
            onPressed: () => _remind(c),
            style: FilledButton.styleFrom(backgroundColor: AppColors.accent, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
...
class _CohortMemberTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = AppLocalizations.of(context)!;
    return InkWell(
      onTap: onTap == null ? null : () => onTap!(customer),
      borderRadius: BorderRadius.circular(AppRadius.lg),
      child: Container(
        margin: const EdgeInsets.only(bottom: 8),
