...
class HomeScreen extends StatelessWidget {
  ...
  void _promptPickBranch(BuildContext context) {
  ...
  Future<void> _openNewBooking(BuildContext context) async {
    ...
    if (data.viewingAllBranches) return _promptPickBranch(context);
  ...
  void _openCustomerProfile(BuildContext context, Map<String, dynamic> booking) {
    final data = DashboardScope.of(context);
  ...
  Future<void> _openRebook(BuildContext context, Map<String, dynamic> booking) async {
    final data = DashboardScope.of(context);
    final salon = _resolveBookingSalon(data, booking);
  ...
  Widget build(BuildContext context) {
    ...
    final data = DashboardScope.of(context);
    final salon = data.currentSalon;
    final title = data.viewingAllBranches ? t.allBranchesLabel : '${salon?['name'] ?? t.salonLabel}';
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    QuickAction(
                      ...
                      onTap: data.viewingAllBranches
                          ? () => _promptPickBranch(context)
                          : () => Navigator.of(context).push(
                                MaterialPageRoute(builder: (_) => ProductsScreen(data: data)),
                              ),
                    ),
                  ],
                ),
                ...
                for (var i = 0; i < logged.length; i++)
                  Padding(
                    ...
                    child: EntranceFade(
                      ...
                      child: BookingLogRow(
                        booking: logged[i],
                        onRebook: () => _openRebook(context, logged[i]),
                        onTap: () => _openCustomerProfile(context, logged[i]),
                      ),
                    ),
                  ),
            ],
